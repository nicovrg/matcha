...
const cleanList = (users, gender, orientation) => {
	switch (orientation) {
		case 'straight' :
			if (gender == 'male') users = users.filter(user => { console.log(user); return (user.gender.name === 'female')});
			if (gender == 'female') users = users.filter(user => { console.log(user); return (user.gender.name === 'male')});
			break;
		case 'gay' :
